...
// @author       AI-generated by SapozhnikUA
...
(function () {
    ...
    // Статус закриття. SDK приймає: 'not-identified' | 'solved' | 'open'
    // Константи: sdk.DataModel.UpdateableMapProblemState.NOT_IDENTIFIED / SOLVED / OPEN
    const CLOSE_AS = 'not-identified';

    // Повідомлення яке надсилається репортеру ПЕРЕД закриттям UR
    const CLOSING_COMMENT = `Вітаємо,
На жаль, ми не отримали достатньо інформації для виправлення помилки.
...
Усі посилання можна знайти на сторінці http://waze.com.ua`;
    ...
    // -------------------------------------------------------------------------
    // Надсилання коментаря через SDK.
    //
    // SDK docs: addComment({ mapUpdateRequestId: number, text: string })
    //   → Promise<ConversationElement>
    //   Throws DataModelNotFoundError якщо сесії коментарів немає в моделі.
    //
    // getUpdateRequestDetails() завантажує details.comments (ConversationElement[]),
    // але НЕ гарантує наявність сесії для addComment. Тому перехоплюємо помилку
    // і якщо сесія відсутня — пропускаємо коментар, але UR все одно закриваємо.
    // -------------------------------------------------------------------------
    async function sendComment(urId) {
        try {
            await sdk.DataModel.MapUpdateRequests.addComment({
                mapUpdateRequestId: urId,
                text: CLOSING_COMMENT,
            });
            log(`  💬 Коментар надіслано до #${urId}`);
        } catch (e) {
            // DataModelNotFoundError — сесія не завантажена, пропускаємо коментар
            log(`  ⚠ Коментар до #${urId} не надіслано (${e?.message}), продовжуємо закриття`);
        }
    }
    ...
    //
    // SDK docs:
    //   getAll() → MapUpdateRequest[]  (масив, не об'єкт!)
    //   MapUpdateRequest: { id, isOpen, isEditable, reportedOn, resolutionState, ... }
    //   getUpdateRequestDetails({ mapUpdateRequestId }) → Promise<UpdateRequestDetails | null>
    //   UpdateRequestDetails: { id, comments: ConversationElement[], driveGeometry }
    //   ConversationElement: { text: string, createdOn: number, userName: string | null }
    // -------------------------------------------------------------------------
    async function runCycle() {
        const now = Date.now();
        const limitMs = DAYS_LIMIT * 24 * 60 * 60 * 1000;

        // getAll() повертає масив MapUpdateRequest[]
        const allUrs = sdk.DataModel.MapUpdateRequests.getAll();
        ...
        for (const ur of openUrs) {
            ...
            try {
                // Завантажуємо деталі UR — якщо null, UR не знайдено
                ...
                // details.comments — ConversationElement[] з полями: text, createdOn, userName
                const comments = details.comments;
                if (comments.length === 0) continue;

                const lastMsg = comments[comments.length - 1];

                // ConversationElement.text — єдине поле з текстом (згідно SDK docs)
                const text = lastMsg.text.replace(/\s+/g, ' ').trim();
                if (!text.includes(TARGET_PHRASE)) continue;

                // ConversationElement.createdOn — timestamp в мілісекундах
                const diffMs = now - lastMsg.createdOn;
                const daysPast = Math.floor(diffMs / 86400000);

                if (diffMs < limitMs) {
                    log(`#${ur.id}: Чекаємо (${daysPast} дн.)`);
                    continue;
                }
                ...
                // Надсилаємо прощальний коментар перед закриттям
                await sendComment(ur.id);

                // updateResolutionState повертає void (не Promise), await не обов'язковий
    ...
    // Зупиняється якщо два цикли поспіль закрили 0 UR, або isRunning = false.
    // -------------------------------------------------------------------------
    ...
    function createUI() {
        ...
        statusLine.style.cssText = `color: #6c8ebf; font-size: 11px; margin-bottom: 2px;`;
        ...
        counter.style.cssText = `color: #4ade80; font-weight: 600; font-size: 11px; margin-top: 4px; display: none;`;
