...
// @version      4.2
// @description  Автозакриття UR через 14 днів иа поовідомлення користувача
...
У випадку її виникнення повторно, або якщо у Вас є зауваження, надішліть, будь ласка, новий звіт.
Приєднуйтесь до наших спільнот у соціальних мережах, де можна швидше отримати допомогу.
Усі посилання опубліковані на сторінці http://waze.com.ua`;
    ...
    // -------------------------------------------------------------------------
    // Надсилання коментаря до UR через W.model.updateRequestSessions.
    // Якщо сесія ще не завантажена — завантажуємо через descartesClient.
    // SDK не має нативного методу для коментарів, тому використовуємо
    // внутрішній WME API (той самий що використовує URComments-Enhanced).
    // -------------------------------------------------------------------------
    async function sendComment(urId, comment) {
        // Завантажуємо сесію якщо вона ще не в моделі
        if (!W.model.updateRequestSessions.getObjectById(urId)) {
            ...
            if (data?.[0]?.updateRequestSessions?.objects?.length > 0) {
                ...
                throw new Error(`Не вдалося завантажити сесію для UR #${urId}`);
            }
        }
        // addComment() ставить коментар у чергу WME і відправляє разом із save()
    ...
    async function runCycle() {
        ...
        for (const ur of openUrs) {
            ...
            try {
                ...
                // Надсилаємо прощальний коментар репортеру перед закриттям
                await sendComment(ur.id, CLOSING_COMMENT);
                log(`  💬 Коментар надіслано до #${ur.id}`);
